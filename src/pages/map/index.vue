...
const skipped=computed(()=>items.value.filter(i=>locationState(i)==='not-needed').length);
...
<view v-else-if="trip" class="screen map-screen"><text class="eyebrow">KNOW YOUR NEXT STOP</text><view class="title">下一站，心里有数。</view><view class="subtitle">{{trip.title}}</view>
<view class="row section"><picker :range="days" :value="days.indexOf(date)" @change="date=days[Number($event.detail.value)];allDays=false"><view class="pill">{{allDays?'选择日期':date}} ▾</view></picker><button class="secondary small" @click="allDays=!allDays">{{allDays?'只看当天':'查看整趟旅行'}}</button></view>
<view class="map-stats section"><view><text class="stat-number">{{located}}</text><text>已有位置</text></view><view><text class="stat-number">{{pending}}</text><text>待补位置</text></view><view><text class="stat-number">{{skipped}}</text><text>无需定位</text></view></view>
...
<view id="map-view" class="section" style="margin-top:12px"><RouteMap large :selection-token="selectionToken" :items="items" :legs="legs" :stays="mapStays" :selected="selected" :pickable="picking" :focus="candidate" @select="selected=$event" @select-stay="go('resources',{id,date,tab:'stay',edit:$event})" @pick="pick"/></view>
