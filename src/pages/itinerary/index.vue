...
function onPlannerListScroll(e:any){const next=Number(e.detail?.scrollTop);if(!Number.isFinite(next))return;const delta=next-observedScrollTop;observedScrollTop=next;if(!dragging.value)listScrollTop.value=next;if(!dragging.value||!delta)return;dragRows=dragRows.map(row=>({top:row.top-delta,bottom:row.bottom-delta}));dragOffset.value=dragPointerY-pressY+next-dragScrollOrigin;if(Math.abs(dragPointerY-pressY)>12)updateDragTarget(dragPointerY);}
...
<view v-if="trip" class="planner" :class="['layout-'+layout,{'no-map':!mapAvailable}]">
 ...
 <view class="planner-sheet" :style="sheetStyle">
  ...
  <scroll-view v-if="showList||dragExpanded" scroll-y :scroll-top="listScrollTop" class="planner-list" :scroll-into-view="listAnchor" :scroll-with-animation="!dragging" :show-scrollbar="false" @scroll="onPlannerListScroll">
