...
import {switchBackend,openCached,changeToken,personalSnapshot,sharedAccess} from '../../state';import {type BackendMode} from '../../services/backend';
...
function downloadBackup(){try{const content=exportBackup(personalSnapshot()),name='xingjian-'+today()+'.json';
// #ifdef H5
...
const backup=ref(''),showBackup=ref(false);const archived=computed(()=>state.data.trips.filter(t=>t.archived&&!sharedAccess[t.id]));
function exportData(){try{backup.value=exportBackup(personalSnapshot());showBackup.value=true;}catch(e){notify(e);}}
...
async function restore(){try{
 if(JSON.parse(backup.value)?.format==='xingjian-resources'){
  const pack=parseResourceSupplement(backup.value);if(sharedAccess[pack.tripId])throw Error('共享旅行请在行程中编辑，不能通过备份导入覆盖');const next=mergeResourceSupplement(personalSnapshot(),pack);
  const before=personalSnapshot().trips.find(t=>t.id===pack.tripId)!,after=next.trips.find(t=>t.id===pack.tripId)!;
...
<view class="section"><view class="section-title">管理旅行</view><view class="row notice" v-for="t in state.data.trips.filter(t=>!sharedAccess[t.id])" :key="t.id"><text>{{t.title}}{{t.archived?' · 已归档':''}}</text><button class="text-button small" @click="archive(t.id,!t.archived)">{{t.archived?'恢复':'归档'}}</button></view></view>
<button class="text-button section" @click="demo">添加一段示例旅行</button><view class="footer-note">行间 0.2 · 认真计划，自在出发</view></view></template>
